...
def draw_bbox(image, bboxes, skip_list, classes=read_class_names(cfg.YOLO.CLASSES), show_label=True):
    ...
    out_boxes, out_scores, out_classes, num_boxes, depths = bboxes
    depth_idx = 0
    for i in range(num_boxes[0]):
        ...
        if show_label:
            if i in skip_list:
                ...
                bbox_mess = '%s: %.2fm' % (classes[class_ind], depths[depth_idx])
                depth_idx += 1
